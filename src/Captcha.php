...
class Captcha
{
    ...
    //签名秘钥
    protected $secretKey = 'yuihjknms63';
    ...
    public function setUseZh($useZh)
    {
        $this->useZh = $useZh;
        return $this;
    }

    public function setUseImgBg($useImgBg)
    {
    ...
    protected function generate(): array
    {
        ...
        $hash = JWT::encode(['key'=>$key,'exp' => time() + $this->expire], $this->secretKey, 'HS256');
     ...
     */
    public function check(string $code, string $captchKey): bool
    {
        ...
        try {
            $res = JWT::decode($captchKey, $key);
        }catch (Exception $e){
            return false;
        }

        if ($res && $res->key == $code && $res->exp > time()) {
            return true;
        }
    ...
    public function create(): array
    {
        ...
        return [
            'captcha' => 'data:image/jpeg;base64,' . base64_encode($content),
